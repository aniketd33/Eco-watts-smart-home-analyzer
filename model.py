...
def prepare_forecast_data(df):
    ...
    daily_data["Day"] = np.arange(len(daily_data))  # Day index for regression
    return daily_data


# ----------------------------------------------------------
# ⚙️ 2. Train Forecast Model
# ----------------------------------------------------------
...
def forecast_next_days(model, daily_data, days_ahead=7):
    ...
    forecast_df = pd.DataFrame({"Date": future_dates, "Predicted_Usage_kWh": future_preds})
    return forecast_df


# ----------------------------------------------------------
# 📈 4. Plot Forecast Results (Plotly)
# ----------------------------------------------------------
def plot_forecast_results(daily_data, forecast_df):
    ...
    fig.add_trace(go.Scatter(
        x=daily_data["Date"], y=daily_data["Usage_kWh"],
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines+markers", name="Forecasted Usage (kWh)",
        line=dict(color="#52b788", dash="dash", width=3)
    ))
    ...
    fig.update_layout(
        ...
        legend=dict(bgcolor="rgba(255,255,255,0.6)", bordercolor="#ccc", borderwidth=1)
    )
    return fig
